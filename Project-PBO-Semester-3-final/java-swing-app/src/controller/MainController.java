...
public class MainController {
    ...
    /**
     * Cari building dari id (untuk dipakai oleh form rating di view)
     */
    public Building getBuildingById(int buildingId) {
        try {
            return buildingDAO.getBuildingById(buildingId);
        } catch (Exception e) {
            System.err.println("✗ Error getBuildingById: " + e.getMessage());
            return null;
        }
    }

    /**
     * Submit rating: insert baru ATAU update jika sudah pernah
     */
    public boolean submitRating(User user, int roomId, int rating, String review) {
        try {
            ...
            // Validasi permission berdasarkan faculty
            ...
            // Cek apakah user sudah pernah rating ruangan ini
            if (ratingDAO.hasUserRatedRoom(user.getUserId(), roomId)) {
                // UPDATE rating
                boolean updated = ratingDAO.updateUserRating(user.getUserId(), roomId, rating, review);
                if (updated) {
                    System.out.println("✓ Rating berhasil diperbarui!");
                } else {
                    System.out.println("✗ Gagal update rating.");
                }
                return updated;
            }

            // Buat objek rating baru
            ...
            if (success) {
                System.out.println("✓ Rating berhasil dikirim: " + rating + " stars for room " + roomId);
            }
